...
const Menu = () => {
    ...
    const getCocktailAt = (indexOffset) => {
        return sliderLists[(currentIndex + indexOffset + totalCoctails) % totalCoctails]
    }
    ...
    return (
        <section id='menu' aria-labelledby='menu-heading'>
            ...
            <nav className='cocktail-tabs' aria-label='Cocktail Navigation'>
                {sliderLists.map(({ id, name, image, title, description }, index) => {
                    const isActive = index === currentIndex;

                    return (
                        <button onClick={() => goToSlide(index)} key={id} className={isActive ? 'text-white border-white' : 'text-white/50 border-white/50'}>{name}</button>
                    )
                })}
            </nav>
